...
Pebbles is the 26th bundled library. Its main form prepares one mesh, uniform scale and collider recipe, then applies it to selected or all clutter types on an exact planet. Imported GLB textures are assigned automatically through every mesh selection path. Common appearance extensions fall back with warnings. Main texture transforms and secondary UV selection are baked automatically; incompatible detail maps are skipped, blended alpha uses cutouts, and alternate image encodings use embedded PNG/JPEG fallbacks when present. Workshop Done updates only the replacement draft; Apply queues native replacement. Live State owns per-body restoration. Presets preserve destination planet/type selections. Loading does not refresh or destroy the preview, rebuild clutter, or change live colliders. See [Pebbles](../pebbles.lib/README.md).

Native acceptance: choose a stock mesh and import a textured GLB through both pickers; fit and rotate compound colliders at scale 2, then resize to 0.5 and verify preview/runtime agreement. Apply to one type, several types and all types; confirm unchecked types remain unchanged, all variants/LODs use the replacement, and no extra random instance scaling appears. Test textured specular/IOR/clearcoat materials, legacy specular/glossiness diffuse textures/factors, unlit/emissive approximations and transformed/secondary UV artwork, skipped detail maps, alpha-blend cutouts and warned wrap approximations. Test PNG/JPEG fallback selection on WebP/KTX2 extensions and a specific missing-decoder error for a main texture without a fallback. Test stationary/moving physics bubbles, destroyed-clutter exclusions, Luna/Mars isolation, repeated Apply/Restore, draft/preset loads while applied, hidden authoring/HUD and unload. Confirm unresolved targets block Apply and no stale GPU descriptors or Bepu shapes remain.

GLB acceptance: browse/load a local self-contained GLB, select complete scene and raw mesh, assign one variant and then all variants, and author colliders before Apply. Test PNG/JPEG materials, base-color factors, alpha masks, mirrored nodes and multiple meshes/materials. Save/load must leave applied bodies and existing preview/cache intact; explicit refresh reopens only matching saved file content. Rename/change/remove the file and verify clear errors when uncached, retained snapshots when cached, and explicit replacement selection. Apply one imported source to two bodies while previewing it, restore one body, then release all and unload; verify no stale textures/bindless slots or lost original clutter. GPU upload/retirement and image decoding require the real native game; managed tests alone do not validate them.
