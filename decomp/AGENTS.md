...
compile the entire solution using standard dotnet cli tools:

`dotnet compile`

this will also copy the built project mods which are distributable to the KSA game mods directory.  if the game is currently running it may cause the build to ultimately fail due to the files being locked and not overwritable
