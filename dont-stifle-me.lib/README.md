...
Relax editor scale and value limits. This feature is hosted by the single **Unscience** mod. Its separate
`dont-stifle-me.lib.csproj` remains the compilation and ownership boundary; there is no standalone entry project.
...
Editor scaling, snapping and expanded-limit policy. Disclosure and authoring scroll state are saved too.
Feature presets retain settings and asset choices while leaving the current target selections intact.

## Live state

The applied editor policy and restore controls. These objects remain owned by this feature and are never serialized into workspace files.
...
- `dont-stifle-me.lib.csproj`: feature assembly and shared infrastructure references. Feature-to-feature project references are prohibited.
...
Authoring/runtime entry files: `DontStifleMeSubmod.Workspace.cs`, `DontStifleMeSubmod.cs`, `DontStifleMeSubmod.Live.cs`.
...
Editor policy starts inactive. Scale and limit hooks activate independently on Apply and are removed on disable. Release restores tracked bounds.
