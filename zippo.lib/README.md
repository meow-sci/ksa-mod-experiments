...
Exact/controlled vehicle and light part, enabled state, intensity/color, animation endpoints, timing and easing. Disclosure and authoring scroll state are saved too.
Feature presets retain settings and asset choices while leaving the current target selections intact.

## Live state

Each managed light and queue; appearance uses the game’s shared template scope. These objects remain owned by this feature and are never serialized into workspace files.
