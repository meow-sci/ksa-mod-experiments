...
Attach and animate sunglasses. This feature is hosted by the single **Unscience** mod. Its separate
`thug-life.lib.csproj` remains the compilation and ownership boundary; there is no standalone entry project.
...
Exact/controlled vehicle, verified part/subpart and transform/size. Disclosure and authoring scroll state are saved too.
Feature presets retain settings and asset choices while leaving the current target selections intact.

## Live state

Each sunglasses attachment and its transform/removal controls. These objects remain owned by this feature and are never serialized into workspace files.
...
- `thug-life.lib.csproj`: feature assembly and shared infrastructure references. Feature-to-feature project references are prohibited.
...
Authoring/runtime entry files: `ThugLifeSubmod.Live.cs`, `ThugLifeSubmod.cs`, `ThugLifeSubmod.Workspace.cs`.
