...
| `TextureLoader.LoadFromMemory` / `Unload`, `Stb.Loader.LoadSettings.ForceRgba8`, decoded `ITexture.Extent/Data` | `Assets/GlbTextures.cs` | Bounded embedded PNG/JPEG decode (including alternate-encoding core fallbacks); detached GlbMaterialReader adapts channel factors, skips incompatible detail maps and converts blended alpha to cutouts. |
...
| `MeshAsset.SetVertexList/SetIndexBuffer`, `NativeStrideList.FromSpan`, bounds/counts; existing `MeshReference` private setters | `Assets/GlbImportLibrary.cs` | Managed GLB scene/accessor validation normalizes external geometry into private float3/float2/uint streams; GlbTextureMapping bakes the main texture’s selected UV set and offset/rotation/scale into MeshAttribute.Uv0, independent of stock global registration. |
...
Pebbles authoring uses one replacement recipe for checked/all clutter types, fixed native instance scale, automatic primitive collision mode and automatic imported material assignment with warned fallbacks for common Blender appearance extensions. Its existing runtime hook/member inventory is unchanged; see [ground clutter](ground-clutter.md#simple-authoring-behavior).
