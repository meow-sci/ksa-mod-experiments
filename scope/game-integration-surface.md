...
| `GetPositionEgo(IPosition) : double3` | direct API | `KSA/Camera.cs:231` | pyro, graffiti | `pyro.lib/PlumeEmitter.cs:72`; `graffiti.lib/DecalPicker.cs`, `DecalAnchors.cs` | OK @5348 | emitter/decal position is camera-ego, like the game's nozzles |
| `NearbyCelestial : Celestial? { get; set; }` | direct API | `KSA/Camera.cs:71` | graffiti | `graffiti.lib/DecalPicker.cs` | OK @5348 | the terrain-march target for a cursor pick that missed every vehicle |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `KittenEva` (type; `GetType().Name=="KittenEva"` / `is KittenEva`) | reflection-type (string, garrys) | `KSA/KittenEva.cs:13` | garrys-torch, doh, kitten-animations, thug-life | `WeldEngine.cs:161`; `KittenSpawner.cs`; `KittenAvatarAccessor.cs`; `KittenGlassesPreset.cs:38` | OK | garrys string-compares the type name (silent break if renamed). thug-life uses a typed `is KittenEva` to gate its **animate thug** preset button — compile-checked, not string-matched |
...
| `InstanceId : uint` | direct API | `KSA/Part.cs:321` | red-alert, graffiti | `ActionScanner.cs:27,30`; `graffiti.lib/GraffitiSubmod.cs` (`FindPart`) | OK | instance addressing; graffiti's stable decal-anchor key (sub-parts included) |
| `RayCastEgo(ref readonly double4x4, Ray, out double ×2, out double3 ×4, out Part? closestSubPart, out Part?) : bool` | direct API | `KSA/Part.cs:2306` | graffiti | `graffiti.lib/DecalPicker.cs` | OK @5348 | KSA's own watertight triangle raycast over the art mesh (the flight-mode hover-pick sweep). Position/normal come back in the **hit sub-part's** local frame; a top-level part with zero sub-parts can never be hit (the sub-part loop never runs) |
| `MatrixAsmb2Ego(in double4x4) : double4x4` | direct API | `KSA/Part.cs` | graffiti | `graffiti.lib/DecalAnchors.cs` | OK @5348 | includes `Part.Scale` and the whole sub-part parent chain — a decal is stuck to the rendered surface, so it wants the full matrix (unlike thug-life, which excludes scale) |
...
| `GetMatrixAsmb2Ego(Camera) : double4x4` · `BoundingSphereRadiusBody : double` · `static ComputeEnu2Cce(double3, doubleQuat) : doubleQuat?` | direct API | `KSA/Vehicle.cs:1202,518,2997` | graffiti | `graffiti.lib/DecalPicker.cs`, `DecalAnchors.cs` | OK @5348 | raycast broad-phase + decal matrix root; ENU basis for terrain anchors (null on the spin axis — pole fallback in `DecalAnchors`) |
...
| `ShaderModuleUtils.FromString(Device, ReadOnlySpan<byte> shaderCode, VkShaderStageFlags, CompileOptions?, ReadOnlySpan<byte> debugName)` | direct (render) | `RenderCore/ShaderModuleUtils.cs:79` (was :77) | humble-arteest (VehiclePaint), graffiti | `VehiclePaintPatches.cs` (`FromFilePrefix`); `graffiti.lib/DecalRenderer.cs` (`Compile`) | OK | `debugName` becomes shaderc's input-file name → relative `#include` resolution. Both mods pass a NUL-terminated real path (the game's own `Utf8.GetBytes` call is not terminated); graffiti's is a fake filename next to the shipped `GridFrag` so `Common/*.glsl` resolves |
