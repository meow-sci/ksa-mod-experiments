...
| `Program._planetTransparenciesRenderer` → `PlanetTransparenciesRenderer._ringsRenderer` | rocky-mcrock-face | two-hop private-field probe (`RingSwapController.IsRingsRendererCreated`) — **status check only**; a rename degrades to a skipped rebuild on auto-apply/dispose, the core swap is unaffected | OK |
