...
- NEW decomp `2026.6.9.4750`: `C:\Users\Alex\repos\meow-sci\ksa-game-assemblies\current\decomp`
- OLD decomp `2026.6.8.4680`: `C:\Users\Alex\repos\meow-sci\ksa-game-assemblies_2026.6.8.4680\current\decomp`
- NEW Content: `C:\Users\Alex\repos\meow-sci\ksa-game-assemblies\current\Content`

`Decomp path (NEW)` is relative to the NEW decomp root (namespace-foldered, e.g. `KSA/Vehicle.cs`).
`Mod code` paths are relative to the repo root `C:\Users\Alex\repos\meow-sci\unscience`.
All five mods **compile clean against NEW (4750)**, so every *typed* member below is implicitly present in NEW; the focus is (a) string-based reflection / asset-id / GLSL-anchor strings that the compiler cannot check, and (b) behavioral/enum/asset deltas.
...
|---|------|----------------------|----------------------------------------|-------------------|---------|----------|-----------|
| 1 | Harmony prefix | `marque/Patcher.cs:39-44` | `GaugeCanvas.OnDrawMenuBar()` — `public static void` | `KSA/GaugeCanvas.cs:809` | Yes | None (OLD `:798`; still static/void/no-args) | Prefix runs inside the View menu; renders `BeginMenu("Marque")`. Static target → empty-arg prefix valid. |
| 2 | Direct API | `marque.lib/MarqueLib.cs:49,52,56,74,77,84,105-117,148-183,216` | `IOrbiter.ShowOrbit` — `ref bool ShowOrbit { get; }` (set) | `KSA/IOrbiter.cs:20` | Yes | None | Toggling via `o.ShowOrbit = true/!show`. `ref bool` property; assignment compiles + works. |
| 3 | Direct API | `marque.lib/MarqueLib.cs:45,195` (via `CelestialProvider`) | `Universe.CurrentSystem.All.UnsafeAsList().OfType<IOrbiter>()` | `KSA/Universe.cs:92`, `KSA/CelestialSystem.cs` | Yes | None | `CurrentSystem` is `CelestialSystem? {get;private set;}`. |
| 4 | Direct API | `marque.lib/MarqueLib.cs:93` | `CelestialSystem.GetWorldSun()` (the Sun `StellarBody`) | `KSA/Universe.cs:173` (call site), `KSA/CelestialSystem.cs` | Yes | None | Root of the celestial tree; `sun.Id`, `sun.Children`. |
| 5 | Direct API (typecheck) | `marque.lib/MarqueLib.cs:56,114,157-159` | `is not Asteroid and not Comet` | `KSA/Asteroid.cs:3`, `KSA/Comet.cs:3` | Yes | None | Both `: MinorBody`. "Planetoids" filter. |
| 6 | Direct API | `marque.lib/MarqueLib.cs:99,101,126-135,223-232` | `IParentBody.Children`, `Celestial`, `Astronomical.Id` | `KSA/Celestial.cs:19` (`: Astronomical,…,IParentBody`), `KSA/Astronomical.cs:11` | Yes | None | Recursive SOI tree walk. |
| 7 | Direct API | `marque.lib/MarqueLib.cs:63,71,84` (via `VehicleProvider`) | `Vehicle.Id`; `Vehicle.ShowOrbit` (as `IOrbiter`) | `KSA/Vehicle.cs` | Yes | None | Vehicles submenu. |
| 8 | Harmony prefix (HotkeyGuard) | `marque/Patcher.cs:17`, `marque/HotkeyGuard.cs:20-22` | `GameSettings.OnKeyAll(GlfwKeyEvent)` — `public static bool` | `KSA/GameSettings.cs:2379` | Yes | None (OLD `:2347`) | NOTE: marque ships a **local copy** of `HotkeyGuard` (`namespace MeowSci.Marque`) instead of the shared `MeowSci.KsaAbstractions.HotkeyGuard`. Functionally identical; deviates from the "use the shared guard" rule. |
...
| 4 | Harmony prefix (HotkeyGuard) | `byo-music/Patcher.cs:19` | `GameSettings.OnKeyAll(GlfwKeyEvent)` — `public static bool` | `KSA/GameSettings.cs:2379` | Yes | None | Uses shared `MeowSci.KsaAbstractions.HotkeyGuard`. |
...
| 26 | Harmony prefix (HotkeyGuard) | `steely-eyed-missile-kitten/Patcher.cs:19` | `GameSettings.OnKeyAll(GlfwKeyEvent)` — `public static bool` | `KSA/GameSettings.cs:2379` | Yes | None | Shared `KsaAbstractions.HotkeyGuard`. |
...
| # | Kind | Mod code (file:line) | Game target (Type.Member + signature) | Decomp path (NEW) | In NEW? | Δ vs OLD | Risk/notes |
|---|------|----------------------|----------------------------------------|-------------------|---------|----------|-----------|
| 1 | Harmony prefix (AccessTools) | `mesh-deform.lib/MeshDeformPatches.cs:34-43` | `PartModelModule.UpdateRenderData(in double4x4, bool, Viewport, int)` | `KSA/PartModelModule.cs:79` | Yes | None | Captures `Part` into a `ThreadLocal`. |
| 2 | Direct API (in prefix) | `mesh-deform.lib/MeshDeformPatches.cs:101` | `PartModelModule.Parent` (Part) | `KSA/PartModelModule.cs:71` | Yes | None | `__instance.Parent`. |
| 3 | Harmony prefix (AccessTools) | `mesh-deform.lib/MeshDeformPatches.cs:51-60` | `PartModel.AddInstance(PerInstanceData, Viewport, int)` | `KSA/PartModel.cs:375` | Yes | None | Single overload → no ambiguity. Prefix takes `ref` first arg to mutate. |
| 4 | Struct reinterpret (`Unsafe.As`) | `mesh-deform.lib/MeshDeformManager.cs:127-134`, `MeshDeformPatches.cs:118-122` | `PartModel.PerInstanceData` = `{float4x4 ModelMatrix; int StateBitFlag; uint EmissiveColor; int packing1; int packing2}` | `KSA/PartModel.cs:299-309` | Yes | None — **byte-identical** struct in both | Reuses `packing1/packing2` (bytes 72-79) as `DeformMagnitude/DeformRadius`. **See finding re: GLSL collision.** |
| 5 | Direct API | `mesh-deform.lib/MeshDeformShaders.cs:43,77` | `PartModelRenderer.ColorData.Rebuild()` — `public static void` | `KSA/PartModelRenderer.cs:17` (`ColorData`) | Yes | None | Rebuilds pipelines after shader swap. |
| 6 | Direct API + asset id | `mesh-deform.lib/MeshDeformShaders.cs:73,136` | `ModLibrary.Get<ShaderReference>("MeshIndirectVert")` | `KSA/ShaderReference.cs:20`; asset reg `Content/Core/DefaultAssets.xml:68` | Yes | None | Asset id `MeshIndirectVert` → file `Shaders/Mesh/MeshIndirect.vert`. Id present in NEW. |
| 7 | Reflection (string) | `mesh-deform.lib/MeshDeformShaders.cs:65-73` | `ShaderReference.DoLoad()` — `internal override void` via `GetMethod("DoLoad", NonPublic\|Public\|Instance)` | `KSA/ShaderReference.cs:167` | Yes | None | Restores original shader on deactivate. |
| 8 | Reflection (string) | `mesh-deform.lib/MeshDeformShaders.cs:232-252` | `ShaderReference.Shader { get; private set; }` setter / fallback field `<Shader>k__BackingField` | `KSA/ShaderReference.cs:33` | Yes | None | Private setter present → primary path used. |
| 9 | Reflection (string) | `mesh-deform.lib/MeshDeformShaders.cs:336` | `ShaderReference.ModPath` (string, on `FileReference` base) | `KSA/ShaderReference.cs:73` (uses `base.ModPath`) | Yes | None | Resolves on-disk shader path. |
| 10 | Direct API | `mesh-deform.lib/MeshDeformShaders.cs:341` | `ShaderReference.LocalPath` | `KSA/ShaderReference.cs:49` (usage) | Yes | None | Fallback path source. |
| 11 | Reflection (string, cross-asm) | `mesh-deform.lib/MeshDeformShaders.cs:373-397` | `RenderCore.ShaderModuleUtils.FromFile(Device, string, out VkShaderStageFlags, CompileOptions)` | `KSA/ShaderReference.cs:108` (call site) | Yes | None | Located by type name `RenderCore.ShaderModuleUtils` + method `FromFile` with `(Device, string, out…)`. |
| 12 | Direct API | `mesh-deform.lib/MeshDeformShaders.cs:39` | `Program.GetRenderer().Device` | `KSA/ShaderReference.cs:98-108`, `KSA/Program.cs` | Yes | None | Vulkan device for compile. |
| 13 | Direct API | `mesh-deform.lib/MeshDeformSubmod.cs:186,282,317-319` | `Vehicle.Parts.Parts`, `Part.DisplayName`, `Part.Id` | `KSA/Vehicle.cs`, `KSA/PartTree.cs`, `KSA/Part.cs` | Yes | None | Part enumeration + labels. |
| 14 | **Asset / GLSL anchor strings** | `mesh-deform.lib/MeshDeformShaders.cs:270-272` (struct), `:277` (main) | `MeshIndirect.vert` literals: `"    uint EmissiveColor;\n};"` **and** `"    vec4 worldPosVec4 = worldMatrix * vec4(inPos, 1.0);"` | `Content/Core/Shaders/Mesh/MeshIndirect.vert` | **Partial** (main anchor present; **struct anchor REMOVED**) | **BREAKING** | See finding below. |
| 15 | Harmony prefix (HotkeyGuard) | `mesh-deform/Patcher.cs:18` | `GameSettings.OnKeyAll(GlfwKeyEvent)` — `public static bool` | `KSA/GameSettings.cs:2379` | Yes | None | Shared guard. |
...
| # | Kind | Mod code (file:line) | Game target (Type.Member + signature) | Decomp path (NEW) | In NEW? | Δ vs OLD | Risk/notes |
|---|------|----------------------|----------------------------------------|-------------------|---------|----------|-----------|
| 1 | Harmony prefix (HotkeyGuard) | `stampy/Patcher.cs:19` | `GameSettings.OnKeyAll(GlfwKeyEvent)` — `public static bool` | `KSA/GameSettings.cs:2379` | Yes | None (OLD `:2347`) | Shared `KsaAbstractions.HotkeyGuard`; the only real game touchpoint. |
