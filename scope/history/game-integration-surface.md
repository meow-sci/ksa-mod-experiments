...
Removed owners: unladen-swallow, steely-eyed-missile-kitten, stampy, space-tape, red-alert, mesh-deform, marque, inanimate-carbon-rod, grant, flexo and blinky. Rows owned only by those features are removed from the active type table. Any remaining source citations to those names in shared rows or dated upgrade notes are historical evidence, not active integrations. In particular BlinkyPatches, the old mesh-deform shader path and all RPC routes are absent.
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `Id : virtual string { get; protected set; }` (via `IObjectId`) | direct API | `KSA/Astronomical.cs:85` |VehicleProvider (→ most mods), eternal-flame, garrys-torch, i-feel-seen, kitchen-sink| `ksa-abstractions.lib/VehicleProvider.cs:22`; `eternal-flame.lib/EternalFlameLib.cs:74` | OK | `Vehicle.Id` resolves here (not declared on `Vehicle`) |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `R / G / B : float` | reflection-field (string) | `KSA/ColorRgbReference.cs:10,13,16` |zippo (unreachable, see Color bug)| `LightController.cs:61-63,82-86`; `red-alert.lib/LightActions.cs:83-86` | OK | red-alert path works; zippo path dead due to `Color` bug |
| `OnDataLoad(Mod) : void` | reflection-method (string) | `KSA/ColorRgbReference.cs:35` |zippo| `LightController.cs:82-86`; `LightActions.cs:83-86` | OK | 1-arg (`new object?[]{null}`); recomputes `Value` |

### KSA.Connection (nested in Part)
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `Connection.Connect(IConnector, IConnector)` | direct API | `KSA/Part.cs:285` |its-so-shiny| `blinky.lib/LcdGridBuilder.cs:352`; `its-so-shiny.lib/ShinyGridBuilder.cs:221` | OK | takes `IConnector` (Part implements), not `(Part,Part)` |
| `Connection.Disconnect()` | direct API | `KSA/Part.cs:301` |its-so-shiny| `LcdGridBuilder.cs:216`; `ShinyGridBuilder.cs:131` | OK | |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `Vehicle.FlightComputer : FlightComputer { get; private set; }` | direct API | `KSA/Vehicle.cs:415` |average-twr| `average-twr.lib/TwrDataReader.cs:17`; `BlinkySubmod.cs:612-618` | OK | |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `OnKeyAll(GlfwKeyEvent) : static bool` | Harmony pre (HotkeyGuard) + reflection-method (`nameof`) | `KSA/GameSettings.cs:2379` |**ALL top-level mods via HotkeyGuard** ( ships a local copy)| `ksa-abstractions.lib/HotkeyGuard.cs:21,23` | OK | suite-wide chokepoint; prefix `ref bool __result` swallows key while typing |
...
| `MeanRadius : double` (via IRadius) | direct API | `KSA/IRadius.cs:5` (steely cite `Vehicle.cs:482`) |average-twr (dead)| `TwrDataReader.cs:11`; `VehicleTelemetry.cs:47` | OK | Ap/Pe altitude (steely); dead path (average-twr) |
| `Children` (add/enumerate) | direct API | `KSA/IParentBody.cs` |doh| `KittenSpawner.cs:174`; `MarqueLib.cs:99,126-135` | OK | spawn parent / SOI tree walk |
...
| `Xkcd.Scarlet`, `Xkcd.PaleGrey : Color.Preset` | direct API | `KSA/KSAColor.cs:1561,837` |garrys-torch, skittles, con-man| `GarrysTorchSubmod.cs:333-334`; `red-alert…RedAlertSubmod.cs:128,129`; `skittles…SkittlesSubmod.cs:108-109`; `ConManSubmod.cs:125-126` | OK | button accents (cosmetic) |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `AccelerationBody : double3` (field, backs `Vehicle.AccelerationBody`) | direct API | `KSA/KinematicMeasurements.cs:9` |geeforce| `geeforce.lib/GForceRecorder.cs:96`; `VehicleTelemetry.cs:81` | OK | body-frame proper accel |
...
| `LightModule` (type, `Get<LightModule>()`) | direct API | `KSA/LightModule.cs` |its-so-shiny (via ZippoLib)| `ActionScanner.cs:68`; `LightActions.cs:51` | OK | |
...
| `TemplateData.ColorRgb : ColorRgbReference` (field) | reflection-field (zippo) / direct (red-alert) | `KSA/LightModule.cs:33` |zippo (intended)| `LightController.cs` (intended); `LightActions.cs:58,71` | OK | red-alert uses it correctly |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `Module.Parent : required Part` | direct API | `KSA/Module.cs:268` |its-so-shiny| `BlinkyPatches.cs:63`; `ShinyPatches.cs:57-63` | OK | `FullPart => PartParent ?? this` |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `Get<T>() : Span<T>` | direct API | `KSA/ModuleList.cs:112` |its-so-shiny, doh, humble-arteest| `ActionScanner.cs:68`; `LcdGridBuilder.cs:327`; `ShinyGridBuilder.cs:205`; `KittenSpawner.cs:278-289`; `EngineEmissive.cs:123` | OK | generic module accessor |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `new Part(string inName, PartTemplate, PartInstance?=null, Part?=null)` (ctor) | direct API | `KSA/Part.cs:765` |its-so-shiny, doh| `LcdGridBuilder.cs:268`; `ShinyGridBuilder.cs:157`; `KittenSpawner.cs:278` | OK | |
| `Id : string { get; init; }` | direct API | `KSA/Part.cs:411` |garrys-torch, zippo, , its-so-shiny, thug-life, kitchen-sink| `GarrysTorchSubmod.cs:188`; `ZippoSubmod.cs`; `ThugLifeSubmod.cs:128`; `MeshDeformSubmod.cs:317-319` | OK | combo labels / pixel-id parsing |
| `DisplayName : string { get; init; }` | direct API | `KSA/Part.cs:413` |zippo| `ZippoSubmod.cs`; `ActionScanner.cs:27`; `MeshDeformSubmod.cs:282` | OK | |
| `Template : PartTemplate` (field) | direct API | `KSA/Part.cs:323` |garrys-torch, zippo, , its-so-shiny, thug-life, kitchen-sink, doh, parts-now| `GarrysTorchSubmod.cs:188`; `LightController.cs:92`; `ThugLifeSubmod.cs:122`; `parts-now.lib/Runtime/RuntimeModUnloadGate.cs:78,148` | OK | feeds reflection/labels; `Template.Id` (SerializedId). parts-now compares it against the record's part ids in the unload safety gate |
| `InstanceId : uint` | direct API | `KSA/Part.cs:321` |graffiti, hot-pursuit| `ActionScanner.cs`; `graffiti.lib/GraffitiSubmod.cs`; `hot-pursuit.lib/HotPursuitSubmod.cs` | OK @5402 | stable sub-part addressing across per-frame target re-resolution |
...
| `FullPart : Part { get; }` | direct API | `KSA/Part.cs:659` |zippo, , its-so-shiny| `ZippoSubmod.cs:152`; `ActionExecutor.cs:80`; `BlinkyPatches.cs:63`; `ShinyPatches.cs:57-63` | OK | `=> PartParent ?? this` |
| `Modules : ModuleList` (field) | direct API | `KSA/Part.cs:401` |humble-arteest| `ActionScanner.cs:68`; `EngineEmissive.cs:123` | OK | `.Get<T>()` / `.Add(...)` |
| `SubtreeModules : ModuleList` (field) | direct API | `KSA/Part.cs:409` |doh| `ActionScanner.cs:50`; `LcdGridBuilder.cs:327`; `KittenSpawner.cs:278-289` | OK | anim/solar/tank discovery |
| `LightSwitch : PowerConsumer?` (field) | direct API | `KSA/Part.cs:407` |zippo, its-so-shiny| `ZippoSubmod.cs:152`; `LightActions.cs:41`; `ShinyPixelCell.cs:24` | OK | light on/off path |
...
| `Connections : List<Connection>` (field) | direct API | `KSA/Part.cs:391` |its-so-shiny| `LcdGridBuilder.cs:214`; `ShinyGridBuilder.cs:133` | OK | |
| `Scale : double3 { get; set; }` | direct API (write) | `KSA/Part.cs:499` |garrys-torch, its-so-shiny| `WeldEngine.cs:200`; `LcdGridBuilder.cs:305`; `ShinyGridBuilder.cs:186` | OK | setter resets cached pos matrix |
| `PositionParentAsmb : double3 { get; set; }` | direct API (write) | `KSA/Part.cs:449` (kitchen-sink cites backing `:333`) |its-so-shiny, kitchen-sink| `LcdGridBuilder.cs:299`; `FlexoPartTest.cs:216` | OK | prefer `:449` (property); kitchen-sink touches the backing field |
| `Asmb2ParentAsmb : doubleQuat { get; set; }` | direct API (write) | `KSA/Part.cs:463` (kitchen-sink cites backing `:337`) |its-so-shiny, kitchen-sink| `LcdGridBuilder.cs:302`; `FlexoPartTest.cs:217` | OK | part rotation write (kitchen-sink's part-move experiment) |
...
| `TreeParent : Part?` | direct API | `KSA/Part.cs:385` |its-so-shiny| `LcdGridBuilder.cs:103-104`; `ShinyGridBuilder.cs:76-77` | OK | manual tree wiring |
| `TreeChildren : List<Part>` (field) | direct API | `KSA/Part.cs:387` |its-so-shiny, kitchen-sink| `LcdGridBuilder.cs:228-230`; `FlexoPartTest.cs:227` | OK | sub-tree collection |
| `SetStage(int)` / `Stage` (get) | direct API | `KSA/Part.cs:731,517` |its-so-shiny| `LcdGridBuilder.cs:124,127`; `ShinyGridBuilder.cs:87` | OK | |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `AddInstance(PerInstanceData, Viewport, int frameIndex) : void` | Harmony pre (humble vehicle-paint, mesh-deform) + post (IvaForceRender) | `KSA/PartModel.cs:375` |humble-arteest (VehiclePaint), IvaForceRender (kitchen-sink)| `VehiclePaintPatches.cs` (`AddInstancePrefix`); `MeshDeformPatches.cs:51-60`; `IvaForceRender.cs:46` | OK | `PartModel.cs` byte-identical; 3-arg single overload. humble binds by param name `instanceData` and ORs paint into `StateBitFlag` |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `UpdateRenderData(in double4x4, bool, Viewport, int)` | Harmony pre | `KSA/PartModelGlassModule.cs:69` |its-so-shiny| `BlinkyPatches.cs:28,32`; `ShinyPatches.cs:27,31` | OK | 4745 merged ModelGlass+ModelEye shaders; C# class unchanged |

### KSA.PartModelModule (+ nested Template, RaytracingMode)
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `UpdateRenderData(in double4x4, bool, Viewport, int)` | Harmony pre (return false skips submit) | `KSA/PartModelModule.cs:79` |its-so-shiny, humble-arteest (VehiclePaint)| `BlinkyPatches.cs:26,30`; `ShinyPatches.cs:25,29`; `MeshDeformPatches.cs:34-43`; `VehiclePaintPatches.cs` (`PartModelModulePrefix`) | OK | game uses `Parent.FullPart.LightSwitch` here; humble reads `Module<T>.Parent : Part` (`KSA/Module.cs:419`); **only caller** of `PartModel.AddInstance` |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `Parts : ReadOnlySpan<Part>` | direct API | `KSA/PartTree.cs:67` |PartHelpers (→ many), garrys-torch, zippo, , its-so-shiny, thug-life, kitchen-sink| `PartHelpers.cs:13`; `ZippoSubmod.cs:406`; `ActionScanner.cs:17` | OK | top-level parts |
| `Root` | direct API | `KSA/PartTree.cs` |its-so-shiny| `LcdGridBuilder.cs:135`; `ShinyGridBuilder.cs:146` | OK | |
...
| `CreateFromNewPartTree(Part rootPart)` | direct API | `KSA/PartTree.cs:117` |its-so-shiny| `LcdGridBuilder.cs:135`; `ShinyGridBuilder.cs:94` | OK | core build path |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `LightIsActive : bool` (field) | direct API | `KSA/PowerConsumer.cs:28` |zippo, its-so-shiny| `ZippoSubmod.cs:161`; `LightActions.cs:42`; `ShinyPixelCell.cs:24,27` | OK | on/off toggle; rev-4681 electrical refactor didn't touch it |
...
| `ControlledVehicle : static Vehicle?` (field) | direct API | `KSA/Program.cs:254` |VehicleProvider (→ average-twr, geeforce, kitten-animations, )| `VehicleProvider.cs:11` | OK | |
...
| Member (signature) | Kind | Decomp path | Used by | Mod code ref(s) | 4750 | Notes |
|---|---|---|---|---|---|---|
| `SimTime` (`readonly struct`) | direct API | `KSA/SimTime.cs:6` |SimTimeProvider (→ geeforce, kiwis-marbles, garrys-torch, doh)| `SimTimeProvider.cs:9` | OK | |
| `Seconds() : double` (instance) | direct API | `KSA/SimTime.cs:67` |geeforce| `GeeForceSubmod.cs:34`; `MonitoringLoop.cs:45` | OK | timestamps sample |
...
| `GetElapsedSimTime() : static SimTime` | direct API | `KSA/Universe.cs:1991` |SimTimeProvider (→ geeforce, kiwis-marbles, doh)| `SimTimeProvider.cs:9` | OK | |
...
| `Parts : PartTree` (field) | direct API | `KSA/Vehicle.cs:264` |PartHelpers (→ many), eternal-flame, its-so-shiny, kitchen-sink| `PartHelpers.cs:13`; `EternalFlameLib.cs:128`; `LcdGridBuilder.cs:37` | OK | get+set (blinky swaps tree) |
...
| `UpdateVehicleConfiguration() : void` | direct API | `KSA/Vehicle.cs:1263` |its-so-shiny| `LcdGridBuilder.cs:149`; `ShinyGridBuilder.cs:98` | OK | |
...
| `Parent : IParentBody => Orbit.Parent` | direct API | `KSA/Vehicle.cs:332` |garrys-torch, average-twr (dead), doh| `WeldEngine.cs:19`; `VehicleTelemetry.cs:44`; `KittenSpawner.cs:230` | OK | |
| `Orbit : Orbit => Patch.Orbit` | direct API | `KSA/Vehicle.cs:330` |garrys-torch| `WeldEngine.cs:126`; `VehicleTelemetry.cs:64` | OK | |
...
| `FlightComputer` (see KSA.FlightComputer) | direct API | `KSA/Vehicle.cs:415` |average-twr| — | OK | |
| `TotalMass : float => _props.TotalMassPropsAsmb.Props.Mass` | direct API | `KSA/Vehicle.cs:512` |average-twr| `TwrDataReader.cs:18`; `VehicleTelemetry.cs:76` | OK | |
| `AccelerationBody : double3 => KinematicMeasurements.AccelerationBody` | direct API | `KSA/Vehicle.cs:518` |geeforce| `GForceRecorder.cs:96`; `VehicleTelemetry.cs:81` | OK | |
...
| `ShaderModuleUtils.FromFile(Device, string filePath, out VkShaderStageFlags shaderStage, CompileOptions? options)` | **Harmony pre** (humble-arteest, free-fallin) + reflection-method (mesh-deform) | `RenderCore/ShaderModuleUtils.cs:115` |humble-arteest (VehiclePaint), free-fallin (Full Canopy)| `VehiclePaintPatches.cs` / `CanopyProjectionShaders.cs` (`FromFilePrefix`); `MeshDeformShaders.cs:373-397` | OK | Shared in-memory shader seam. Humble targets two part fragments; free-fallin targets `Model.vert`, `Model_Skinned.vert`, and `ModelPbr.frag`; both pass through all other paths and fall back to stock on error. Param names `device`/`filePath`/`shaderStage`/`options` are load-bearing for Harmony binding. |
...
| `ModPath` (on `FileReference` base, public property) | direct API | `KSA/FileReference.cs:23` |humble-arteest| `VehiclePaintShaders.cs` (`TryResolveShaderPath`); `MeshDeformShaders.cs:336,341` | OK | resolve on-disk shader path (humble: pre-flight anchor check only) |
