...
| 9 | `ModLibrary.AllMeshes : SerializedCollection<MeshReference>` / `ModLibrary.AllFiles : SerializedCollection<FileReference>` (internal static fields) | **reflection (string)** | `KSA/ModLibrary.cs:80,68` | `RingAssetCatalog.Collection<T>` — same pattern/names as parts-now `GameRegistry` (already on the watchlist) | OK |
