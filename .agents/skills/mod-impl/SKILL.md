...
- For bundled features, retain or create a separate feature .lib.csproj and register IWorkspaceFeature with the existing unscience host. Do not create standalone entry projects. Use the fixme templates only when explicitly creating a separate nonshipping mod.
...
- MUST update ksa-mod-experiments.slnx for new shipping feature/shared library projects and update all documentation required by AGENTS.md.
- MUST follow the task instructions precisely for implemting the mod
- MAY use exploratory learning from the the sibling `../ksa-game-assemblies/current/decomp` folder which contains decompiled ksa dll assemblies.  some of these files are large so be careful not to fill up context, use #runSubagent agents to do exploration as needed
...
- MUST keep feature-specific behavior, authoring UI and runtime inspectors in the owning .lib project. The single host owns shared workspace UI and lifecycle. Bind detached authoring data explicitly and expose live items separately; save/load must not affect applied runtime state. Feature-to-feature project references and RPC distribution are retired.
