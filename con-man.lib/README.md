...
Configure the game gauge layout. This feature is hosted by the single **Unscience** mod. Its separate
`con-man.lib.csproj` remains the compilation and ownership boundary; there is no standalone entry project.
...
Detached gauge visibility, offsets and scales; selected legacy layout. Disclosure and authoring scroll state are saved too.
Feature presets retain settings and asset choices while leaving the current target selections intact.

## Live state

The live console layout, legacy saving and startup layout controls. These objects remain owned by this feature and are never serialized into workspace files.
...
- `con-man.lib.csproj`: feature assembly and shared infrastructure references. Feature-to-feature project references are prohibited.
...
Authoring/runtime entry files: `ConManSubmod.cs`, `ConManSubmod.Workspace.cs`, `ConManSubmod.Live.cs`.
...
Applying a layout captures the affected gauge objects’ original enabled flags, offsets and scales. Release/unload restores those fields and window geometry. A saved preferred layout is not applied at startup.
